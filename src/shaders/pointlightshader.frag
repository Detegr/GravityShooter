...
vec4 pointLight(vec3 pos, vec3 normal)
{
	...
	vec4 lightColor2 = (ambient + diffuse + specular);
	float attenuation = 1.0 + 1.0 * distance + 1.0 * distance * distance;

	return lightColor2 / attenuation;
}
